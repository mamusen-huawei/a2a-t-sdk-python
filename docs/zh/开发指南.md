...
服务端在 A2A 业务回调中，从消息扩展字段取出 processed task prompt，传递给 `A2ATServer.check_task_prompt` 校验：
...
def handle_a2at_message(message: dict) -> dict:
    processed_prompt = message["extensions"].get(NOTIFICATION_PROMPT_EXT)
...
def handle_message(
    ...
    processed_prompt = message.get("extensions", {}).get(NOTIFICATION_PROMPT_EXT)
    if not processed_prompt:
        return {"error": "missing A2A-T task prompt"}
